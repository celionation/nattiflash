...
<form action="" method="get" id="pagerForm" onsubmit="return false;">
    <div class="d-flex justify-content-center align-items-center my-5">
        ...
        <button class="btn btn-sm btn-info" <?= $canBack ? "" : "disabled" ?> onclick="pager(<?= $page - 1 ?>)"> Prev </button>
        <div class="mx-3"><?= $page ?> / <?= $totalPages ?></div>
        <button class="btn btn-sm btn-info" <?= $canForward ? "" : "disabled" ?> onclick="pager(<?= $page + 1 ?>)"> Next </button>
    </div>
</form>
